...
def main():
    ...
    results["black"] = run_command("black --check app/ config/", "Black (Code Formatter)", critical=False)
    results["isort"] = run_command("isort --check-only app/ config/", "isort (Import Sorter)", critical=False)

    # 2. Code Quality
    print_header("2️⃣  CODE QUALITY")
    results["pylint"] = run_command("pylint app/ --exit-zero", "Pylint (Code Linter)", critical=False)

    # 3. Security Scanning
    print_header("3️⃣  SECURITY SCANNING")
    results["bandit"] = run_command(
        "bandit -r app/ -ll", "Bandit (SAST - Static Application Security Testing)", critical=True
    )
    results["safety"] = run_command("safety check", "Safety (SCA - Software Composition Analysis)", critical=False)

    # 4. Unit Tests
    print_header("4️⃣  UNIT TESTS")
    results["unit_tests"] = run_command("pytest tests/ -m unit -v", "Unit Tests", critical=True)

    # 5. Integration Tests
    print_header("5️⃣  INTEGRATION TESTS")
    results["integration_tests"] = run_command("pytest tests/ -m integration -v", "Integration Tests", critical=False)

    # 6. Coverage
    print_header("6️⃣  CODE COVERAGE")
    run_command("coverage run -m pytest tests/ -m unit", "Running tests with coverage", critical=False)
    results["coverage"] = run_command("coverage report --fail-under=80", "Coverage Report (minimum 80%)", critical=True)
