...
def run_command(command, shell=True):
    ...
    try:
        result = subprocess.run(
            command,
            shell=shell,  # nosec B602 - commands are hardcoded, not user-controlled
            capture_output=True,
            text=True,
            timeout=10,
        )
...
def check_current_system():
    ...
    checks = [
        ...
        (
            "4. User Model",
            "python -c \"from config.settings import AUTH_USER_MODEL; print(f'User Model: {AUTH_USER_MODEL}')\"",
        ),
        (
            "5. Installed Apps",
            "python -c \"from config.settings import INSTALLED_APPS; print('\\n'.join(INSTALLED_APPS))\"",
        ),
    ]
    ...
    try:
        ...
        print("  Modelo actual: UserAccount")
