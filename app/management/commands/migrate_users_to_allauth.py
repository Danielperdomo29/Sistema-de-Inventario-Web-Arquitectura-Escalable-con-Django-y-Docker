...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--user-id',
            type=int,
            help='ID de usuario específico a migrar'
        )
    ...
    def handle(self, *args, **options):
        ...
        if not users.exists():
            self.stdout.write(
                self.style.WARNING('No hay usuarios para migrar')
            )
            return
        ...
        for user in users:
            try:
                ...
                errors += 1
                self.stdout.write(
                    self.style.ERROR(f'✗ Error migrando {user.email}: {str(e)}')
                )
        
        # Resumen
        self.stdout.write('\n' + '=' * 60)
        self.stdout.write(
            self.style.SUCCESS(f'✅ {migrated} usuarios migrados exitosamente')
        )
        if errors > 0:
            self.stdout.write(
                self.style.ERROR(f'❌ {errors} errores durante la migración')
            )
