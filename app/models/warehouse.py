...
class Warehouse(models.Model):
    ...
    def get_all():
        ...
        return list(Warehouse.objects.filter(activo=True).values('id', 'nombre', 'ubicacion', 'capacidad').order_by('nombre'))
    
    @staticmethod
    def get_by_id(warehouse_id):
        """Obtiene un almacén por su ID"""
        try:
            return Warehouse.objects.filter(id=warehouse_id, activo=True).values('id', 'nombre', 'ubicacion', 'capacidad').first()
        except Exception:
            return None
