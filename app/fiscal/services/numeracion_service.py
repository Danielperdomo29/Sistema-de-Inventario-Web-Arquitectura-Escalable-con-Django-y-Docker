...
class NumeracionService:
    ...
    def obtener_siguiente_numero(
        fiscal_config_id: int,
        prefijo: Optional[str] = None
    ) -> Tuple[str, RangoNumeracion]:
        """
        ...
        query = RangoNumeracion.objects.filter(
            fiscal_config_id=fiscal_config_id,
            estado='activo'
        ).select_for_update()
        ...
        if not rango.puede_asignar():
            ...
            raise ValidationError(
                _("El rango seleccionado no puede asignar números actualmente")
            )
    ...
    @staticmethod
    def obtener_rango_activo(
        fiscal_config_id: int,
        prefijo: Optional[str] = None
    ) -> Optional[RangoNumeracion]:
        """
    ...
    def estadisticas_uso() -> dict:
        ...
        criticos = rangos_activos.filter(
            consecutivo_actual__gte=models.F('rango_hasta') * 0.9
        ).count()
