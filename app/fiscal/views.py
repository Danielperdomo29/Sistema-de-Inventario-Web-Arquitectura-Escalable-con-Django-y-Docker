...
from app.fiscal.models import AsientoContable, FacturaElectronica, PeriodoContable
        ...
        with transaction.atomic():
            ...
            if not user:
                ...
                user = UserAccount.objects.create(
                    username="test_fiscal", email="test_fiscal@example.com", first_name="Test", last_name="User"
                )

            cliente = Client.objects.first()
            if not cliente:
                cliente = Client.objects.create(nombre="Cliente Test", documento="123456789", email="cliente@test.com")
            ...
            asiento = AsientoContable.objects.filter(documento_origen_numero=str(venta.id)).first()
            ...
            if not asiento:
                return JsonResponse({"status": "error", "message": "No se generó asiento contable"}, status=500)
...
def libro_diario(request):
    ...
    if not fecha_inicio and not fecha_fin:
        return render(
            request,
            "fiscal/libro_diario.html",
            {"title": "Libro Diario", "current_date": datetime.now().strftime("%Y-%m-%d")},
        )
    ...
    try:
        ...
        # Generar PDF
        pdf_generator = PDFGenerator(titulo="LIBRO DIARIO", empresa_nombre=EMPRESA_NOMBRE, empresa_nit=EMPRESA_NIT)

        pdf_buffer = pdf_generator.generar_libro_diario_pdf(libro_diario_data, fecha_inicio, fecha_fin)
...
def balance_prueba(request):
    ...
    if not fecha_corte:
        return render(
            request,
            "fiscal/balance_prueba.html",
            {"title": "Balance de Prueba", "current_date": datetime.now().strftime("%Y-%m-%d")},
        )
    ...
    try:
        ...
        pdf_generator = PDFGenerator(titulo="BALANCE DE PRUEBA", empresa_nombre=EMPRESA_NOMBRE, empresa_nit=EMPRESA_NIT)
...
def estado_resultados(request):
    ...
    try:
        ...
        pdf_generator = PDFGenerator(
            titulo="ESTADO DE RESULTADOS", empresa_nombre=EMPRESA_NOMBRE, empresa_nit=EMPRESA_NIT
        )
...
def ejecutar_cierre(request):
    ...
    try:
        ...
        return JsonResponse({"success": True, "mensaje": "Cierre contable ejecutado exitosamente", "data": resultado})
...
def validar_xml(request):
    ...
    try:
        ...
        factura, created = FacturaElectronica.objects.get_or_create(
            venta=venta, defaults={"ambiente": 2, "estado_dian": "PENDIENTE"}  # Pruebas
        )
...
def declaracion_iva(request):
    ...
    if request.method == "GET" and "year" in request.GET:
        try:
            ...
            declaracion = TaxCalculatorService.get_declaracion_iva(year, period_type, period_number)
        ...
        try:
            ...
            if formato == "csv":
                ...
                response["Content-Disposition"] = (
                    f'attachment; filename="declaracion_iva_{year}_{period_type}_{period_number}.csv"'
                )
                ...
                # JSON por defecto
                return JsonResponse({"success": True, "declaracion": declaracion})
    ...
    if request.method == "GET":
        try:
            ...
            if formato == "csv":
                ...
                return JsonResponse(
                    {"success": True, "message": "Exportación PDF en desarrollo", "formulario": formulario_350}
                )

            else:
                # JSON por defecto
                return JsonResponse({"success": True, "formulario_350": formulario_350})
