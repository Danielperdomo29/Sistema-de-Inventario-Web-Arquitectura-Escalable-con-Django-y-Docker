...
class UBLMapper:
    ...
    def map_sale_to_ubl_data(
        sale: Sale, fiscal_config: "FiscalConfig", numero_factura: str, cufe: str
    ) -> Dict[str, Any]:
    ...
    def _calcular_impuestos(sale):
        ...
        for detalle in detalles:
            ...
            if tasa_str not in iva_por_tasa:
                iva_por_tasa[tasa_str] = {"base": Decimal("0"), "valor": Decimal("0"), "tasa": tasa}
        ...
        return {
            "iva": {
                ...
                "detalles": [
                    {"tasa": float(item["tasa"]), "base": float(item["base"]), "valor": float(item["valor"])}
