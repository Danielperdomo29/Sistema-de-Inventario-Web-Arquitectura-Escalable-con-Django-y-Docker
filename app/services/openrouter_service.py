...
class OpenRouterService:
    ...
    FAILOVER_CHAIN = [
        "nvidia/nemotron-3-nano-30b-a3b:free",  # 30B params (potente)
        "arcee-ai/trinity-large-preview:free",  # Modelo grande en preview
        "stepfun/step-3.5-flash:free",  # Rápido y eficiente
        "liquid/lfm-2.5-1.2b-thinking:free",  # Pequeño pero con capacidad de razonamiento
        "liquid/lfm-2.5-1.2b-instruct:free",  # Versión instruct del anterior
        "arcee-ai/trinity-mini:free",  # Modelo pequeño para respuestas simples
        "google/gemini-2.0-flash-lite-preview-02-05:free",  # Fallback conocido
        "meta-llama/llama-3.2-3b-instruct:free",  # Fallback conocido
    ]
    ...
    def __init__(self):
        ...
        if not self.api_key:
            raise ValueError(
                "OPENROUTER_API_KEY no está configurada. " "Por favor, configura tu API key en el archivo .env"
            )
        self.headers = {
