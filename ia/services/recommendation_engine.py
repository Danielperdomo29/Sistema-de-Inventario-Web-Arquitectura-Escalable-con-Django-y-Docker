...
class RecommendationEngine:
    ...
    def obtener_recomendaciones_completas(self, user_id: int = None) -> Dict[str, Any]:
        """
        ...
        """
        recomendaciones = {"timestamp": timezone.now().isoformat(), "resumen": {}, "categorias": {}}
    ...
    def recomendar_liquidacion(self) -> Dict[str, Any]:
        ...
        try:
            ...
            productos_con_ventas = (
                SaleDetail.objects.filter(venta__fecha__gte=fecha_corte)
            ...
            sin_rotacion = (
                Product.objects.filter(stock_actual__gt=10)  # Al menos 10 unidades
                .exclude(id__in=productos_con_ventas)
                .values("id", "nombre", "codigo", "stock_actual", "precio_compra", "precio_venta")[:20]
            )
    ...
    def sugerir_ajustes_precio(self) -> Dict[str, Any]:
        ...
        try:
            ...
            productos_analizados = (
                SaleDetail.objects.filter(venta__fecha__gte=fecha_inicio)
                .values("producto_id", "producto__nombre", "producto__precio_compra", "producto__precio_venta")
                .annotate(cantidad_vendida=Sum("cantidad"))[:30]
            )

            sugerencias = []

            for prod in productos_analizados:
                precio_compra = float(prod["producto__precio_compra"] or 0)
                precio_venta = float(prod["producto__precio_venta"] or 0)
    ...
    def obtener_productos_relacionados(self, producto_id: int, limite: int = 5) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            ventas_con_producto = SaleDetail.objects.filter(producto_id=producto_id).values_list("venta_id", flat=True)

            # Encontrar otros productos en esas ventas
            productos_relacionados = (
                SaleDetail.objects.filter(venta_id__in=ventas_con_producto)
                .exclude(producto_id=producto_id)
                .values("producto_id", "producto__nombre")
                .annotate(frecuencia=Count("id"))
                .filter(frecuencia__gte=self.MIN_COMPRAS_JUNTAS)
            ...
            return {
                ...
                "relacionados": [
                    {"producto_id": p["producto_id"], "nombre": p["producto__nombre"], "frecuencia": p["frecuencia"]}
