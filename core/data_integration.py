...
class DataAggregator:
    ...
    def _obtener_datos_analytics(self, fecha_inicio, fecha_fin) -> Dict[str, Any]:
        """Obtiene datos estructurados para módulo Analytics"""
        try:
            from app.models import Product, Sale

            # Datos de ventas del período
            ventas = Sale.objects.filter(fecha__range=[fecha_inicio, fecha_fin]).exclude(
                estado__in=["ANULADA", "CANCELADA"]
            )

            # Agregaciones
            ventas_agg = ventas.aggregate(total=Sum("total"), cantidad=Count("id"), promedio=Avg("total"))
            ...
            return {
                ...
                "ventas_por_dia": [
                    {"fecha": str(v["fecha__date"]), "total": float(v["total"] or 0), "cantidad": v["cantidad"]}
    ...
    def _obtener_top_productos(self, fecha_inicio, fecha_fin, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            productos = (
                Product.objects.filter(saledetail__venta__fecha__range=[fecha_inicio, fecha_fin])
                .annotate(
    ...
    def _obtener_stats_inventario(self) -> Dict[str, Any]:
        ...
        try:
            ...
            bajo_stock = Product.objects.filter(stock_actual__lte=F("stock_minimo")).count()
            sin_stock = Product.objects.filter(stock_actual=0).count()
    ...
    def _obtener_alertas_urgentes(self, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            alertas = AlertaAutomatica.objects.filter(estado="PENDIENTE").order_by("-fecha_creacion")[:limit]

            return [
                {
                    "id": a.id,
                    "titulo": getattr(a, "titulo", a.tipo_alerta),
                    "mensaje": a.mensaje,
                    "tipo": a.tipo_alerta,
    ...
    def obtener_contexto_para_consulta(
        self, intencion: str, usuario_id: Optional[int] = None, limite: int = 5
    ) -> Dict[str, Any]:
        """
        ...
        """
        contexto = {"intencion": intencion, "timestamp": timezone.now().isoformat(), "datos": {}}
    ...
    def _contexto_ventas(self, fecha: Any, limite: int) -> Dict[str, Any]:
        ...
        try:
            ...
            # Ventas de hoy
            ventas_hoy = (
                Sale.objects.filter(fecha__gte=inicio_dia).exclude(estado="ANULADA").order_by("-fecha")[:limite]
            )
            ...
            return {
                ...
                "resumen": {"total_ventas_hoy": float(total_hoy), "cantidad_hoy": ventas_hoy.count()},
            }
        except Exception as e:
    ...
    def _contexto_inventario(self, limite: int) -> Dict[str, Any]:
        ...
        try:
            ...
            bajo_stock = Product.objects.filter(stock_actual__lte=F("stock_minimo")).order_by("stock_actual")[:limite]
            ...
            return {
                ...
                "resumen": {"productos_bajo_stock": bajo_stock.count(), "productos_sin_stock": sin_stock},
            }
        except Exception as e:
    ...
    def _contexto_financiero(self, fecha: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            ventas_mes = (
                Sale.objects.filter(fecha__gte=inicio_mes)
                .exclude(estado="ANULADA")
                .aggregate(total=Sum("total"), cantidad=Count("id"))
            )

            return {
                "ventas_mes": {"total": float(ventas_mes["total"] or 0), "cantidad": ventas_mes["cantidad"] or 0},
    ...
    def _contexto_prediccion(self, fecha: Any, limite: int) -> Dict[str, Any]:
        ...
        return {"nota": "Predicciones estarán disponibles en Fase 3 (Analytics)", "fecha_consulta": fecha.isoformat()}

    def health_check(self) -> Dict[str, Any]:
        """Verifica estado del DataAggregator"""
        checks = {"kpi_service": False, "sale_model": False, "product_model": False, "alert_model": False}
