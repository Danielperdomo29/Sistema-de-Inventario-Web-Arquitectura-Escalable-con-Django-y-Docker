...
class DemandForecaster:
    ...
    def entrenar_modelo(
        self, producto_id: int = None, dias_historico: int = 365, forzar: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        if not forzar and cache_key in self.modelos:
            return {"exito": True, "mensaje": "Modelo ya existe en cache", "desde_cache": True}

        # Obtener datos del ETL
        dataset = self.etl.load_dataset_entrenamiento(producto_id=producto_id, dias=dias_historico)

        if not dataset["exito"]:
            logger.warning(f"Datos insuficientes para producto {producto_id}: {dataset['mensaje']}")
            return {"exito": False, "mensaje": dataset["mensaje"], "usar_fallback": True}
        ...
        try:
            ...
            # Feature importance
            feature_importance = dict(zip(dataset["feature_names"], modelo.feature_importances_))

            # Guardar modelo
            self.modelos[cache_key] = {
                "modelo": modelo,
                "feature_names": dataset["feature_names"],
                "metricas": {"mae": mae, "r2": r2, "confianza": confianza, "error_percentil_90": error_percentil_90},
            ...
            return {"exito": False, "mensaje": "sklearn no instalado", "usar_fallback": True}
        except Exception as e:
            logger.error(f"Error entrenando modelo: {e}")
            return {"exito": False, "mensaje": str(e), "usar_fallback": True}

    def predecir_demanda(self, producto_id: int, dias_futuros: int = 30) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {
                ...
                "intervalo_error": {"tipo": "percentil_90", "valor": round(error_base, 1)},
                "modelo": {
                    "tipo": "RandomForest",
                    "r2": metricas["r2"],
    ...
    def _predecir_promedio_movil(self, producto_id: int, dias_futuros: int) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            ventas = self.etl.extract_ventas_historicas(dias=self.DIAS_PROMEDIO_MOVIL, producto_id=producto_id)

            if not ventas:
                return {"exito": False, "mensaje": "Sin datos históricos", "producto_id": producto_id}
            ...
            return {
                ...
                "intervalo_error": {"tipo": "desviacion_estandar", "valor": round(desviacion, 1)},
                "modelo": {
                    "tipo": "PromedioMovil",
        ...
        try:
            ...
            if filepath.exists():
                with open(filepath, "rb") as f:
                    self.modelos[cache_key] = pickle.load(f)  # nosec B301 - trusted internal model
    ...
    def get_productos_con_modelo(self) -> List[int]:
        ...
        return [int(p.stem.replace("producto_", "")) for p in modelos_en_disco]
