...
class ETLProcessor:
    ...
    def extract_ventas_historicas(
        self, dias: int = None, fecha_inicio: date = None, fecha_fin: date = None, producto_id: int = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            # Query base
            ventas_qs = (
                SaleDetail.objects.filter(venta__fecha__range=[fecha_inicio, fecha_fin])
                .exclude(venta__estado__in=["ANULADA", "CANCELADA"])
                .select_related("venta", "producto")
            )

            # Filtrar por producto si se especifica
            if producto_id:
                ventas_qs = ventas_qs.filter(producto_id=producto_id)
            ...
            for detalle in ventas_qs.iterator():
                ventas.append(
                    {
                        "fecha": (
                            detalle.venta.fecha.date() if hasattr(detalle.venta.fecha, "date") else detalle.venta.fecha
                        ),
    ...
    def transform_para_forecasting(
        self, ventas_raw: List[Dict[str, Any]], agregacion: str = "diaria"
    ) -> Dict[str, Any]:
        """
    ...
    def _agrupar_ventas(self, ventas: List[Dict], agregacion: str) -> List[Dict[str, Any]]:
        """Agrupa ventas por período"""
        agrupado = defaultdict(lambda: defaultdict(lambda: {"cantidad": 0, "total": 0, "transacciones": 0}))
    ...
    def load_dataset_entrenamiento(self, producto_id: int = None, dias: int = 365) -> Dict[str, Any]:
        """
        ...
        """
        # Extract
        ventas_raw = self.extract_ventas_historicas(dias=dias, producto_id=producto_id)
        ...
        if not datos:
            return {"exito": False, "mensaje": "No hay datos después de limpieza", "X": [], "y": []}
    ...
    def get_datos_para_prediccion(self, fecha_inicio: date, fecha_fin: date) -> List[Dict[str, Any]]:
        """
