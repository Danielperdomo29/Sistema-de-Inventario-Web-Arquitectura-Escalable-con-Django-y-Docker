...
class InventoryOptimizer:
    ...
    def optimizar_producto(
        self, producto_id: int, nivel_servicio: float = None, lead_time_dias: int = None, costo_pedido: float = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not prediccion["exito"]:
                return {"exito": False, "mensaje": "No se pudo predecir demanda", "producto_id": producto_id}
            ...
            if demanda_diaria <= 0:
                return {"exito": False, "mensaje": "Demanda predicha es cero o negativa", "producto_id": producto_id}
            ...
            eoq = self._calcular_eoq(demanda_anual, costo_pedido, costo_almacen)

            # Factor Z para nivel de servicio
            z = self._get_z_factor(nivel_servicio)

            # Stock de seguridad
            stock_seguridad = self._calcular_stock_seguridad(demanda_diaria, intervalo_error, lead_time_dias, z)

            # Punto de reorden
            punto_reorden = self._calcular_punto_reorden(demanda_diaria, lead_time_dias, stock_seguridad)
            ...
            recomendaciones = self._generar_recomendaciones(stock_actual, punto_reorden, eoq, stock_minimo_actual)
    ...
    def clasificacion_abc(self, top_n: int = 100) -> Dict[str, Any]:
        ...
        try:
            ...
            ventas = (
                SaleDetail.objects.filter(venta__fecha__gte=fecha_inicio)
                .values("producto_id", "producto__nombre")
                .annotate(cantidad_total=Sum("cantidad"), valor_total=Sum(F("cantidad") * F("precio_unitario")))
            ...
            # Calcular totales
            valor_total_general = float(sum(float(v["valor_total"] or 0) for v in ventas))
            ...
            return {
                ...
                "periodo": {"inicio": fecha_inicio.isoformat(), "fin": date.today().isoformat()},
            }
    ...
    def _calcular_eoq(self, demanda_anual: float, costo_pedido: float, costo_almacen: float) -> float:
        """
    ...
    def _calcular_stock_seguridad(
        self, demanda_diaria: float, desviacion: float, lead_time_dias: int, z: float
    ) -> float:
        """
        Stock de Seguridad
    ...
    def _calcular_punto_reorden(self, demanda_diaria: float, lead_time_dias: int, stock_seguridad: float) -> float:
        """
        Punto de Reorden
    ...
    def _generar_recomendaciones(
        self, stock_actual: int, punto_reorden: float, eoq: float, stock_minimo_actual: int
    ) -> List[str]:
